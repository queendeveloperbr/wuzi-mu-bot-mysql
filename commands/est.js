...
blur(6)
 ...
 //const pxAtual = pxFixal - name.length
  ...
  ctx.stroke()
    ...
    await client.connection.query(`SELECT * FROM eco WHERE id = ${pessoa.id}`, (err, row) => {
    

  ctx.fillStyle = "#4B0082"
  ctx.globalAlpha = 0.6
  ctx.fillRect(210, 54, ((100 / (row[0].lvl * 30)) * row[0].xp) * 3, 29)
  ctx.fill()
  ctx.globalAlpha = 1

  ctx.font = "18px Arial"
  ctx.fillStyle = "#FFFFFF"
  ctx.fillText(`${row[0].xp} / ${row[0].lvl * 30} XP`, 300, 76)
  ...
  ctx.font = "extra bold 30px Sans"
  ctx.fillStyle = '#4B0082'
  ctx.fillText(row[0].lvl, 310, 126)
})
